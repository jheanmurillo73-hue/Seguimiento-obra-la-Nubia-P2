...
export interface PipeConduit {
  ...
  metersPresupuestados?: number | string;
  metersEjecutados?: number | string;
  isEjecutado?: boolean;
}
...
export const getConduitPresupuestadoMeters = (
  conduit: Partial<PipeConduit>,
  fallbackMeters: number = 0,
): number => {
  const raw = conduit.metersPresupuestados !== undefined ? conduit.metersPresupuestados : conduit.meters;
  const num = typeof raw === 'number' ? raw : parseFloat(String(raw || '0').replace(',', '.'));
  return Number.isFinite(num) && num >= 0 ? num : fallbackMeters;
};

export const isConduitEjecutado = (conduit: Partial<PipeConduit>): boolean => {
  if (typeof conduit.isEjecutado === 'boolean') {
    return conduit.isEjecutado;
  }
  if (conduit.metersEjecutados !== undefined) {
...
export const getConduitEjecutadoMeters = (
  ...
  if (conduit.isEjecutado === false) return 0;
  if (conduit.metersEjecutados !== undefined) {
    const num = typeof conduit.metersEjecutados === 'number'
      ? conduit.metersEjecutados
      : parseFloat(String(conduit.metersEjecutados || '0').replace(',', '.'));
  ...
  return getConduitPresupuestadoMeters(conduit, fallbackMeters);
};

export const normalizePipeConduits = (
  value: unknown,
  legacy?: Partial<Pick<PipeConduit, 'networkType' | 'configuration' | 'meters' | 'metersPresupuestados' | 'metersEjecutados' | 'isEjecutado'>>,
): PipeConduit[] => {
  ...
  const conduits = candidates.reduce<PipeConduit[]>((items, candidate, index) => {
    ...
    const rawPresup = item.metersPresupuestados !== undefined ? item.metersPresupuestados : item.meters;
    const metersPresupuestados = typeof rawPresup === 'number' || (typeof rawPresup === 'string' && rawPresup.trim())
      ? rawPresup
      : 0;

    const isEjecutado = typeof item.isEjecutado === 'boolean'
      ? item.isEjecutado
      : (item.metersEjecutados !== undefined ? parseFloat(String(item.metersEjecutados)) > 0 : true);

    const metersEjecutados = !isEjecutado
      ? 0
      : (item.metersEjecutados !== undefined ? item.metersEjecutados : metersPresupuestados);

    const meters = metersPresupuestados;
  ...
  const legacyPresup = legacy.metersPresupuestados !== undefined ? legacy.metersPresupuestados : (legacy.meters ?? 0);
  const legacyIsEjec = typeof legacy.isEjecutado === 'boolean' ? legacy.isEjecutado : true;
  const legacyEjec = !legacyIsEjec ? 0 : (legacy.metersEjecutados !== undefined ? legacy.metersEjecutados : legacyPresup);
  ...
  return [{
    ...
    meters: legacyPresup,
    metersPresupuestados: legacyPresup,
    metersEjecutados: legacyEjec,
    isEjecutado: legacyIsEjec,
  }];
};
 ...
 * - totalLinearMeters: Cantidad presupuestada fija (base contractual de diseño). Intacta.
 * - ejecutadoLinearMeters: Cantidad ejecutada real en campo (afectada por checklist Ejecutado / No Ejecutado).
 */
export const getPhotoRealLinearMeters = (photo: InspectionPhoto): {
  totalLinearMeters: number;
  ejecutadoLinearMeters: number;
  distanceMeters: number;
  distanceEjecutadaMeters: number;
  multiplier: number;
} => {
  const conduits = photo.pipeConduits;
  if (conduits && conduits.length > 0) {
    let totalLinearPresup = 0;
    let totalLinearEjec = 0;
    let totalDistPresup = 0;
    let totalDistEjec = 0;
    let maxMult = 1;
    ...
    conduits.forEach((c) => {
      ...
      totalLinearEjec += mult * ejec;
      totalDistPresup += presup;
      totalDistEjec += ejec;
    ...
    return {
      totalLinearMeters: Math.round(totalLinearPresup * 100) / 100,
      ejecutadoLinearMeters: Math.round(totalLinearEjec * 100) / 100,
      distanceMeters: Math.round(totalDistPresup * 100) / 100,
      distanceEjecutadaMeters: Math.round(totalDistEjec * 100) / 100,
      multiplier: maxMult,
    };
  }

  const basePresup = photo.metersPresupuestados !== undefined
    ? parseFloat(String(photo.metersPresupuestados).replace(',', '.')) || 0
    : parseFloat(String(photo.metraje || '0').replace(',', '.')) || 0;
  
  let baseEjec: number;
  if (photo.isEjecutado === false) {
    baseEjec = 0;
  } else if (photo.metersEjecutados !== undefined) {
    baseEjec = parseFloat(String(photo.metersEjecutados).replace(',', '.')) || 0;
  } else {
    baseEjec = basePresup * (getPhotoProgressPercentage(photo) / 100);
  }

  const mult = extractTramoMultiplier(photo.tramo);
  return {
    totalLinearMeters: Math.round(mult * basePresup * 100) / 100,
    ejecutadoLinearMeters: Math.round(mult * baseEjec * 100) / 100,
    distanceMeters: Math.round(basePresup * 100) / 100,
    distanceEjecutadaMeters: Math.round(baseEjec * 100) / 100,
    multiplier: mult,
  };
};
