                        ...
                        return (
                          <>
                            ...
                            <line
                              ...
                              x2={photo.planEndX}
                              y2={photo.planEndY}
                              stroke={actaTheme.colorHex}
                              strokeWidth={isPending ? "1.8" : "2.4"}
              ...
              if (type === 'tuberia' || isCable(photo)) {
                ...
                return (
                  <React.Fragment key={photo.id}>
                    ...
                    {arePipeNamesVisible && (
                      <span
                        className={`pointer-events-none absolute z-20 max-w-[160px] truncate rounded-md border border-indigo-500/35 bg-white/95 px-1.5 py-1 font-mono text-[9px] font-bold text-indigo-800 shadow-[0_3px_10px_rgba(55,48,163,0.22)] ${isDimmed ? 'opacity-20' : ''}`}
                    ...
                    {areActaLabelsVisible && photo.showActaLabel !== false && (actaName || mapColorMode === 'actas') && (
                      <span
                        className={`pointer-events-none absolute z-20 flex max-w-[165px] items-center gap-1 whitespace-nowrap rounded-md border px-1.5 py-1 font-mono text-[9px] font-bold shadow-sm ${
                          mapColorMode === 'actas'
                            ? ''
                            : 'border-[#0b5d8c]/35 bg-white/95 text-[#0b4770]'
                        } ${isDimmed ? 'opacity-20' : ''}`}
                        style={{
                          ...getActaLabelStyle(midpointX, midpointY, photo.actaLabelPosition, 4 + iconScale * 16, textScale),
                          ...(mapColorMode === 'actas' ? {
                            backgroundColor: actaTheme.bgLight,
                            borderColor: actaTheme.borderHex,
                            color: actaTheme.textHex,
                            boxShadow: `0 2px 8px ${actaTheme.glowColor}`,
                          } : {})
                        }}
                        title={actaName ? `${actaName} (${actaTheme.estado})` : 'Pendiente por Facturar'}
                      >
                        <span className="material-symbols-outlined text-[13px]">{actaTheme.estado === 'Facturado' ? 'verified' : isPendingActa ? 'hourglass_top' : 'assignment'}</span>
                        <span className="truncate">{actaName || 'Sin Acta'}</span>
                        {mapColorMode === 'actas' && (
                          <span
                            className="text-[7.5px] uppercase tracking-wider px-1 py-0.5 rounded font-sans border"
                            style={{ backgroundColor: 'rgba(255,255,255,0.9)', borderColor: actaTheme.borderHex, color: actaTheme.textHex }}
                          >
                            {actaTheme.estado === 'Facturado' ? 'Fact.' : actaTheme.estado === 'En Revisión' ? 'Rev.' : 'Pend.'}
                          </span>
                        )}
                      </span>
                    )}
              ...
              const baseMarkerColor = electrical
                ...
                : photo.cameraType === 'BT'
                  ...
                  : photo.cameraType === 'Datos'
                    ? '#f97316'
                    : '#0566aa';
              const markerColor = mapColorMode === 'actas'
                ? actaTheme.colorHex
                : isNotStarted(photo)
                  ? NO_STARTED_MARKER_COLOR
                  : baseMarkerColor;
              ...
              return (
                <React.Fragment key={photo.id}>
                  ...
                  {cameraName && areCameraNamesVisible && (
                    <span
                      className={`pointer-events-none absolute z-20 max-w-[160px] truncate rounded-md border border-[#0566aa]/35 bg-white/95 px-1.5 py-1 font-mono text-[9px] font-bold text-[#075a91] shadow-[0_3px_10px_rgba(6,36,58,0.24)] ${isDimmed ? 'opacity-20' : ''}`}
                  ...
                  {areActaLabelsVisible && photo.showActaLabel !== false && (actaName || mapColorMode === 'actas') && (
                    <span
                      ...
                      style={{
                        ...getActaLabelStyle(photo.planX!, photo.planY!, photo.actaLabelPosition, 4 + iconScale * 18, textScale),
                        ...(mapColorMode === 'actas' ? {
                          backgroundColor: actaTheme.bgLight,
                          borderColor: actaTheme.borderHex,
                          color: actaTheme.textHex,
