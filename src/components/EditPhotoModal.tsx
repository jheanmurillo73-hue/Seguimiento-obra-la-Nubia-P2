...
export const EditPhotoModal: React.FC<EditPhotoModalProps> = ({
  ...
  const updatePipeConduit = (id: string, changes: Partial<PipeConduit>) => {
  ...
  const handlePhotoChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validFiles: File[] = selectedFiles.filter((file: File) => file.type.startsWith('image/'));
    ...
    try {
      const filesToProcess: File[] = validFiles.slice(0, remainingSlots);
      const optimizedImages = await Promise.all(filesToProcess.map((file: File) => compressEvidenceImageForUpload(file)));
      const originalSize: number = filesToProcess.reduce((total: number, file: File) => total + file.size, 0);
      const optimizedSize: number = optimizedImages.reduce((total: number, image) => total + image.optimizedBytes, 0);
              ...
              return (
                <div key={conduit.id} className="rounded-lg border border-[#c7dce7] bg-white p-3 shadow-[0_1px_0_rgba(7,63,116,0.05)]">
                  ...
                  <TramoSelector
                    ...
                    onMetrajeChange={(meters) => updatePipeConduit(conduit.id, { meters, metersPresupuestados: meters })}
                    metersPresupuestados={conduit.metersPresupuestados !== undefined ? conduit.metersPresupuestados : conduit.meters}
                    onMetersPresupuestadosChange={(metersPresupuestados) => {
                      updatePipeConduit(conduit.id, {
                        metersPresupuestados,
                        meters: metersPresupuestados,
                      });
                    }}
                    metersEjecutados={conduit.metersEjecutados !== undefined ? conduit.metersEjecutados : (conduit.isEjecutado === false ? 0 : conduit.meters)}
                    onMetersEjecutadosChange={(metersEjecutados) => {
                      const num = parseFloat(String(metersEjecutados));
                      updatePipeConduit(conduit.id, {
                        metersEjecutados,
                        isEjecutado: Number.isFinite(num) && num > 0 ? true : conduit.isEjecutado,
                      });
                    }}
                    isEjecutado={conduit.isEjecutado !== false}
                    onIsEjecutadoChange={(isEjecutado) => {
                      updatePipeConduit(conduit.id, {
                        isEjecutado,
                        metersEjecutados: isEjecutado
                          ? (conduit.metersEjecutados !== undefined && Number(conduit.metersEjecutados) > 0
                              ? conduit.metersEjecutados
                              : (conduit.metersPresupuestados || conduit.meters || 0))
                          : 0,
                      });
                    }}
                    showPresupuestoVsEjecutado={true}
                    maxQuantity={option.value === 'baja_tension' ? 21 : 24}
                    label={`Calibre y seguimiento · ${option.label}`}
                  />
                </div>
              );
            })}
          </div>
          <div className="rounded-lg border border-[#9fc7d9] bg-white px-3.5 py-3 shadow-xs">
            <div className="flex items-center justify-between mb-2">
              <p className="font-mono text-[11px] font-bold uppercase tracking-wide text-[#527284] flex items-center gap-1.5">
                <span className="material-symbols-outlined text-[15px] text-[#004d99]">table_chart</span>
                Resumen de Tuberías y Avance Físico
              </p>
              <span className="text-[10px] font-bold text-[#004d99] bg-[#e6f6ff] px-2 py-0.5 rounded border border-[#004d99]/20">
                Presupuesto Invariable
              </span>
            </div>
            <div className="space-y-1.5">
              {orderedPipeConduits.map((conduit) => {
                const opt = getPipeNetworkOption(conduit.networkType);
                const presup = getConduitPresupuestadoMeters(conduit);
                const ejec = getConduitEjecutadoMeters(conduit);
                const ejecStatus = isConduitEjecutado(conduit);
                const pct = presup > 0 ? Math.min(100, Math.round((ejec / presup) * 100)) : 0;
                return (
                  <div key={conduit.id} className="flex flex-wrap items-center justify-between gap-2 p-2 rounded-lg bg-[#f8fafc] border border-slate-200">
                    <div className="flex items-center gap-2">
                      <span className="h-2.5 w-2.5 rounded-full shrink-0" style={{ backgroundColor: opt.color }} />
                      <span className="font-['Inter'] font-bold text-[12px] text-[#073f74]">
                        Tubería {opt.label} {conduit.configuration || 'sin medida'}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
